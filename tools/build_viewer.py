...
<div id="screen-library">
  ...
  <div id="lib-body">
    <div id="lib-empty">
      ...
      <p class="lib-hint">Tap a route below, or load a <code>.jumroutebook</code> file.</p>
    </div>
    <div id="catalogue"></div>
    <div id="lib-list-header" class="hidden" style="max-width:640px;margin:0 auto .6rem;font-size:.7rem;font-weight:700;color:#718096;text-transform:uppercase;letter-spacing:.07em;">My Routes</div>
    <div id="add-row">
...
<div id="map-modal">
  <div id="map-modal-inner">
    <div id="map-modal-banner">
      <div id="map-modal-banner-main">
        <span id="map-modal-banner-title"></span>
      </div>
      <div id="map-modal-banner-body">
        <span id="map-modal-banner-stats"></span>
      </div>
    </div>
...
#open-btn:active { background: #2c5282; }
...
#url-row button:active, #add-row button:active { background: #2c5282; }
...
#catalogue-heading {
  font-size: .7rem; font-weight: 700; color: #718096;
...
.cat-item-title { font-weight: 700; font-size: .97rem; color: #1a365d; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.cat-item-meta { font-size: .78rem; color: #718096; margin-top: 1px; }
.cat-btn {
  flex-shrink: 0; border: none; border-radius: 8px;
  padding: .4rem .85rem; font-size: .82rem; font-weight: 700;
  cursor: pointer; white-space: nowrap; transition: background .15s;
}
.cat-btn-install { background: #2b6cb0; color: white; }
.cat-btn-install:active { background: #2c5282; }
.cat-btn-update  { background: #d97706; color: white; }
.cat-btn-update:active  { background: #b45309; }
.cat-btn-open    { background: #edf2f7; color: #2b6cb0; }
.cat-btn-open:active    { background: #e2e8f0; }
...
#map-modal-leaflet { flex: 1; min-height: 0; }
#map-modal-banner {
  background: rgba(255,255,255,.96);
  border-bottom: 1px solid #e2e8f0;
  z-index: 410; flex-shrink: 0;
}
#map-modal-banner-main {
  display: flex; align-items: center;
  padding: .55rem .9rem;
}
#map-modal-banner-title {
  font-weight: 700; font-size: .92rem; color: #2d3748; flex: 1; min-width: 0;
  white-space: nowrap; overflow: hidden; text-overflow: ellipsis;
}
#map-modal-banner-body {
  padding: .3rem .9rem .55rem; font-size: .82rem; color: #4a5568;
  display: flex; flex-wrap: wrap; gap: .4rem .9rem;
}
#map-modal-banner-body .stat { white-space: nowrap; }
#map-modal-close {
  position: absolute; top: 12px; right: 12px; z-index: 420;
...
if ('serviceWorker' in navigator) {
  navigator.serviceWorker.addEventListener('message', function(e) {
    if (e.data && e.data.type === 'UPDATE_AVAILABLE') {
      ...
      banner.onclick = function() { window.location.href = window.location.href.split('?')[0]; };
    }
  });
}
...
async function renderLibrary() {
  ...
  const listHeader = document.getElementById('lib-list-header');
  list.innerHTML = '';

  renderCatalogue(routes);
...
function renderCatalogue(routes) {
  ...
  el.innerHTML = '<div id="catalogue-heading">Available Routes</div>';
  for (const entry of CATALOGUE) {
    const installed = routes.find(r => r.sourceUrl === entry.url);
    const hasUpdate = installed && document.querySelector('#lib-list li[data-id="' + CSS.escape(installed.id) + '"] .update-pill');
    const div = document.createElement('div');
    div.className = 'cat-item';
    div.innerHTML =
      '<div class="cat-item-body">' +
        '<div class="cat-item-title">' + escHtml(entry.title) + '</div>' +
        '<div class="cat-item-meta">' + escHtml(entry.meta) + '</div>' +
      '</div>';
    if (!installed) {
      const btn = document.createElement('button');
      btn.className = 'cat-btn cat-btn-install';
      btn.textContent = '+ Install';
      btn.addEventListener('click', () => _loadUrl(entry.url));
      div.appendChild(btn);
    } else {
      const openBtn = document.createElement('button');
      openBtn.className = 'cat-btn cat-btn-open';
      openBtn.textContent = '▶ Open';
      openBtn.addEventListener('click', () => openRoute(installed));
      div.appendChild(openBtn);
      // Check for update asynchronously
      checkCatalogueUpdate(entry, div);
    }
    el.appendChild(div);
  }
}

async function checkCatalogueUpdate(entry, div) {
  try {
    ...
    const installed = routes.find(r => r.sourceUrl === entry.url);
    if (!installed) return;
    if (mf.content_hash && installed.manifest.content_hash && mf.content_hash !== installed.manifest.content_hash) {
      // Insert update button before the open button
      const updateBtn = document.createElement('button');
      updateBtn.className = 'cat-btn cat-btn-update';
      updateBtn.textContent = '↻ Update';
      updateBtn.addEventListener('click', () => _loadUrl(entry.url));
      const openBtn = div.querySelector('.cat-btn-open');
      div.insertBefore(updateBtn, openBtn);
    }
...
function openMapModal(sec) {
  ...
  // Populate banner
  const banner = document.getElementById('map-modal-banner');
  document.getElementById('map-modal-banner-title').textContent = 'Sec ' + String(sec.index).padStart(2,'0') + ': ' + (sec.label || (sec.start_name + ' → ' + sec.end_name));
  const ss = sec.surface_stats || {};
  const surfParts = [
    ss.asphalt  > 0 ? '🟢 ' + ss.asphalt  + '% asphalt'  : null,
    ss.gravel   > 0 ? '🟠 ' + ss.gravel   + '% gravel'   : null,
    ss.unpaved  > 0 ? '🔴 ' + ss.unpaved  + '% unpaved'  : null,
  ].filter(Boolean).join('  ');
  const kmRange = (sec.start_km != null && sec.end_km != null)
    ? '<span class="stat">🗺 ' + Math.round(sec.start_km) + '–' + Math.round(sec.end_km) + ' km</span>' : '';
  document.getElementById('map-modal-banner-stats').innerHTML =
    '<span class="stat">📏 ' + sec.distance_km + ' km</span>' +
    kmRange +
    '<span class="stat">↑ ' + (sec.elevation_gain_m || 0) + ' m</span>' +
    '<span class="stat">↓ ' + (sec.elevation_loss_m || 0) + ' m</span>' +
    (surfParts ? '<span class="stat">' + surfParts + '</span>' : '');
  ...
  // Draw full route in neutral color underneath the section
  if (currentBook && currentBook.handbook && currentBook.handbook.sections) {
    const allPts = [];
    currentBook.handbook.sections.forEach(function(s) {
      if (s.track_points) s.track_points.forEach(function(p) { allPts.push([p.la, p.lo]); });
    });
    if (allPts.length > 1) {
      L.polyline(allPts, {color:'#f6883a', weight:3, opacity:.6}).addTo(map);
    }
  }
