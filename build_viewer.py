...
const PRIORITY_OPEN = new Set(['restaurant','cafe','fast_food','bar','pub',
...
function renderSection(sec, assets) {
  ...
  if (sec.notable_turns && sec.notable_turns.length) {
    const cards = sec.notable_turns.map(t => {
      ...
      const imgHtml = thumb
        ? '<a href="' + escHtml(photo.full_url||'#') + '" target="_blank"><img src="' + thumb + '" class="turn-photo" alt="Photo"></a>'
        : '<div class="turn-photo-na">📷 No photo</div>';
      const mapsHref = 'https://www.google.com/maps?q=' + t.lat + ',' + t.lon;
  ...
  if (sec.waypoints && sec.waypoints.length) {
    ...
    const groupHtml = sorted.map(cat => {
      ...
      const rows = wps.map(wp => {
        const gm = 'https://www.google.com/maps?q=' + wp.lat + ',' + wp.lon;
